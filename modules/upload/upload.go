...
import (
	...
	"github.com/lafriakh/kira/modules/helpers"
)
...
func New(c *kira.Context) *Upload {
	...
	return &Upload{
		size:     c.Config().GetInt64("server.body_limit", 32) * MB,
		request:  c.Request(),
		config:   c.Config(),
		required: true,
	}
...
func (u *Upload) Upload() (Uploaded, error) {
	...
	if !u.isImage {
		...
		uploaded, err = writeFile(u.config, file, path)
		if err != nil {
			return Uploaded{}, err
		}
	} else {
		...
		if u.imageTo != "" {
			...
			uploaded, err = writeImage(u.config, file, path, u.imageTo)
			if err != nil {
				return Uploaded{}, err
			}
		} else {
